...
class Bus:
    ...
    def find_downstream_impacts(self):
        ...
        def check(dev, current_mask):
            ...
            if hasattr(dev, "is_ph_closed"):
                for ph in "abc":
                    if not dev.is_ph_closed(ph): new_mask[ph] = False
            elif hasattr(dev, "is_closed") and not dev.is_closed:
                for ph in "abc": new_mask[ph] = False
            if not any(new_mask.values()): return
            
            attrs = ["connections"]
            if dev.__class__.__name__ == "PowerTransformer": attrs = ["x_connections"]
            elif hasattr(dev, "x_connections") and not hasattr(dev, "h_connections"): attrs = ["x_connections"]
            elif hasattr(dev, "h_connections"): attrs = ["x_connections", "connections"]
            
            for attr in attrs:
                for c in getattr(dev, attr, []): check(c, new_mask)
    ...
    def current(self):
        ...
        if getattr(self, '_evaluating_i', False): return wye_currents()
        self._evaluating_i = True
        try:
            v_sys = self.voltage
            if not v_sys or not v_sys.is_energized(): return wye_currents()

            loads, faults = self.find_downstream_impacts()
            if not loads and not faults: return wye_currents()
