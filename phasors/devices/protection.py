    ...
    def current(self):
        ...
        try:
            ...
            for src in self.inputs:
                ...
                if i_sys:
                    total_a += i_sys.a.to_complex(); total_b += i_sys.b.to_complex(); total_c += i_sys.c.to_complex()
                    found = True
            if not found: return None
            def from_c(c): mag, ang = cmath.polar(c); return CurrentPhasor(mag, math.degrees(ang))
            return wye_currents(from_c(total_a), from_c(total_b), from_c(total_c))
    ...
    def voltage(self):
        ...
        try:
            ...
            for src in self.inputs:
                ...
                if v_sys:
                    total_a += v_sys.a.to_complex(); total_b += v_sys.b.to_complex(); total_c += v_sys.c.to_complex()
            ...
            return wye_voltages(from_c(total_a), from_c(total_b), from_c(total_c))
        finally: self._evaluating = False
        ...
        try:
            ...
            for i, src in enumerate(self.inputs):
                ...
                if i_sys:
                    ...
                    if self.mode == "DIFFERENTIAL" and i > 0 and src.name not in self.input_polarities: sign = -1
...
class Relay(ProtectionDevice):
    def __init__(self, name: str, function: str = "Differential", input_polarities: dict = None, category: str = "Numerical",
        ...
        self.logic = logic or {"TRIP": "50P1 OR IN101", "OUT101": "50P1"}
    ...
    def get_terminal_state(self, label=None) -> bool:
        ...
        if not label or label == "DC_OUT": label = "TRIP" # Default to TRIP for legacy calls
        ...
        try:
            ...
            return self._evaluate_equation(eq, self.get_logic_bits())
        finally:
            self._evaluating_dc = False
    ...
    def get_summary_dict(self):
        ...
        for out in self.digital_outputs + (["TRIP"] if "TRIP" not in self.digital_outputs else []):
