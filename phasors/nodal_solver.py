...
phase-A phasor.  Phases B and C are derived by ±120° rotation.
...
# Phase-B/C rotation factors (positive-sequence: A=0°, B=+120°, C=+240°)
_ROT_B = cmath.exp(1j * math.radians(120))
_ROT_C = cmath.exp(1j * math.radians(240))
...
def _run_solver(primary: list, np) -> bool:
    ...
    # 4. Build Y-bus matrix.
    Y = _build_ybus(reachable, primary_set, key_idx, N, np)

    # 5. Iterative solve (3 passes of Gauss-Seidel PQ update).
    V = _iterative_solve(Y, node_keys, key_idx, reachable, slack_idx, N, np)
    if V is None:
        return False

    # 6. Write solved voltages into device caches.
    _write_cache(V, node_keys, key_idx, reachable)
    return True
